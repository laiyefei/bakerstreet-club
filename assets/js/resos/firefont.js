  ...
  Sakri.CanvasTextUtil.getFontSizeForWidth = function(string, fontProps, width, canvas, fillStyle, maxFontSize){
    if(!canvas){
      var canvas = document.createElement("canvas");
            canvas.setAttribute("id", sCanvasId);
    }
    if(!fillStyle){
      fillStyle = "#000000";
    }
...
function commitResize(){
    ...
    if(htmlBounds.width >= maxStageWidth){
       ...
       // canvas.style.left = -(htmlBounds.getCenterX() - (maxStageWidth/2))/1.5+"px";
    ...
    if(htmlBounds.height > maxStageHeight){
        ...
        canvas.style.top = -canvas.height/3 +"px";
    }else{
    ...
    var oHfWidth = document.getElementById("hfCanvasWidth");
    var oHfHeight = document.getElementById("hfCanvasHeight");
    var oHfTop = document.getElementById("hfCanvasTop");
    var oHfLeft = document.getElementById("hfCanvasLeft");
    if(null != oHfWidth)
    {
          oHfWidth.value = canvas.width;
    }
    if(null != oHfHeight)
    {
          oHfHeight.value = canvas.height;
    }
    if(null != oHfTop)
    {
          oHfTop.value = -canvas.height/3;
    }
    if(null != oHfLeft)
    {
          oHfLeft.value = - canvas.width/8;
    }
